...
//           dispatch(setPosts(updatedPosts)); // Update Redux store
//           toast.success(res.data.message);
//       }
//   } catch (error) {
//       console.error(error);
...
//         <div className="flex items-center gap-3">
//           {
//             liked ? <FaHeart onClick={ likeOrdislikeHandler} size={'22'} className="cursor-pointer text-red-600"/> :
...
//             dispatch(setSelectedPost(post));
//              setOpen(true);
//           }} className="text-blue-500 cursor-pointer text-sm text-gray-400">
...
//           className="outline-none text-sm w-full border-none"
//         />
...
const Post = ({ post }) => {
  ...
  //   try {
  //     const res = await axios.get(`https:localhost:8080/api/v1/post/${post?._id}/bookmark`, {withCredentials:true} );
  //     if(res.data.success) {
  //       toast.success(res.data.message)
  //     }
  //   } catch (error) {
  ...
  return (
    <div className="my-8 w-full max-w-sm mx-auto">
      ...
      <span className="font-medium block mb-2">{postLike} likes</span>
      <p>
        <span className="font-medium mr-2">{post.author?.username || "User"}</span>
        {post.caption}
      </p>
